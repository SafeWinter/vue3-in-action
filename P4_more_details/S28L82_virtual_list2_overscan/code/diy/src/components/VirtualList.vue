...
const initPostions = () => {
  ...
  positions = props.listData.map((_, index) => ({
    index, // 列表项的下标
    height: estimated, // 列表项的高度，这里采用预估的高度
    top: index * estimated, // 列表项的顶部位置，根据下标和预估高度计算
    bottom: (index + 1) * estimated // 列表项的底部位置，也是根据下标和预估高度计算
  }))
}
...
const aboveCount = computed(() => {
  ...
  return Math.min(startIndex.value, Math.ceil(props.bufferScale * visibleCount.value))
})
...
const createObserver = () => {
  observer.value = new IntersectionObserver(
    ...
    {
      root: parentRef.value, // 设置观察的根元素为列表容器
...
// 观察列表项
// visibleEls 就是一个一个列表项的引用
const observeItems = () => visibleEls.value.forEach((el) => observer.value.observe(el))

onMounted(() => {
  // 获取可视区域高度
  screenHeight.value = parentRef.value.clientHeight
  startIndex.value = 0
  endIndex.value = startIndex.value + visibleCount.value
...
const updateItemsSize = () => {
  visibleEls.value.forEach((node, index) => {
    // 获取列表项实际的高度
...
const setStartOffset = () => {
  ...
  // 设置偏移量
  contentWrapper.value.style.transform = `translateY(${startOffset}px)`
}

onUpdated(() => {
  // 这里之所以使用 nextTick，是为了确保DOM更新完毕后再去获取列表项的位置信息
  nextTick(() => {
    if (!visibleEls.value || !visibleEls.value.length) return
    // 1. 更新列表项的高度
    updateItemsSize()
    // 2. 更新虚拟列表的高度
