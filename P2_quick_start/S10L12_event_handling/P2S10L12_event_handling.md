...
在 `Vue` 中如果要给元素绑定事件，可以使用内置指令 `v-on`：
...
1. 如果事件相关的处理比较简单，那么可以将事件处理器写成内联形式：
...
- 没有显式传参：事件对象会作为一个额外的参数，自动传入到事件处理器，在事件处理器中，只需要在形参列表中声明一下即可；
- 如果显式传参：这种情况下需要使用一个特殊的变量 `$event` 来向事件处理器传递事件对象：
...
> `Event Modifiers` 官方文档：https://vuejs.org/guide/essentials/event-handling.html#event-modifiers

之所以会有修饰符，是因为之前在书写原生事件处理逻辑时，经常会包含诸如阻止冒泡、阻止默认事件等非事件业务的逻辑。有了修饰符后，可以利用事件修饰符来完成这些非核心业务处理，让事件处理器更加专注于业务逻辑。
...
另外需要说一下，事件修饰符是 **可以连用** 的，例如现在有这么一个需求，我们希望用户在点击按钮时：
...
- `.enter`
- `.tab`
- `.delete` (捕获 <kbd>Del</kbd> 和 <kbd>Backspace</kbd> 两个按键)
- `.esc`
- `.space`
- `.up`
- `.down`
...
而添加了 `.exact` 修饰符之后，只有在按下 `ctrl` 且没有按下其他按键时才会触发事件：
