<template>
  <div class="h-full w-full fixed overflow-hidden flex">
    <div class="h-full flex-none w-72 bg-gray-800 text-gray-50 p-3 flex flex-col">
      ...
      <ul class="mt-10 px-10 text-center">
        <li v-for="(item, index) in routes" class="mb-10" :key="index">
          ...
          <button @click="handleAddPage(item.name)" class="ml-5 text-gray-500 focus:outline-none">
            +
          </button>
        </li>
      </ul>
      ...
      <div class="flex items-center p-5 border-b text-gray-500">
        ...
        <ul class="flex">
          <li
            ...
            :key="pageName"
          >
            <router-link active-class="text-yellow-600" :to="{ name: pageName }">
              {{ pageName }}
            </router-link>
            <button
...
const handleAddPage = addPage
const handleRemovePage = removePage
</script>
