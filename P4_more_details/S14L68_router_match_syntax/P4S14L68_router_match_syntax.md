...
1. 静态路由匹配：`/about` :arrow_right: `/about`；
2. 动态路由匹配：`/users/:id` :arrow_right: `/users/1`、`/users/2`；
...
我们期望后面是一个用户的 `id`，如 `/users/1`、`/users/2`
...
例如，路由 `/users` 匹配：`/users`、`/users/`、甚至 `/Users/`。
...
最后官方推荐了一个 [路径排名调试工具 Path Ranker](https://paths.esm.dev/?p=AAMeJSyAwR4UbFDAFxAcAGAIJXMAAA..)，可以用于了解为什么一个路由没有被匹配，或者报告一个 `bug`。
