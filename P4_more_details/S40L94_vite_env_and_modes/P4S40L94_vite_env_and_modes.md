...
- 开发环境：可能使用具有限制权限的 `API` 密钥，这些密钥用于防止在开发过程中对真实数据造成影响。
...
一言以蔽之：在 `Vite` 中，模式是一个可以在启动时设置的属性，**用来指导 Vite 如何加载 .env 文件**。

`.env` 文件 **用来存放环境变量**，之后在客户端源码中就可以通过 `import.meta.env` 来访问对应的值。
...
之后使用 `Vite` 启动应用就可以指定特定的模式，从而加载不同的 `.env` 文件：
...
> **思考**
>
> 为什么有了 `NODE_ENV` 还需要 `mode` 模式？
>
> 答：`NODE_ENV` 是在 `Node.js` 环境中最早被支持的一个环境变量，广泛用于各大工具和库。而模式是特定于某个构建工具，主要就是用于更加细粒度的场景控制。也就是说，使用 `mode` 可以在不影响 `NODE_DEV` 的前提下具备更高的灵活性。
>