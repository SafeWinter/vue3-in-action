...
这里就定义了一个 `Document` 接口，其内部有一个 `createElement` 方法，用于创建 `DOM` 元素。

接下来 **浏览器开发者** 基于 `C++` 实现上述接口：
...
下一步非常重要：需要生成 **绑定代码（绑定层）**；它绑定了 `JS` 如何调用这个 `C++` 方法：
...
这里我们不需要考虑同属于 `JS` 层面的计算，去深究解析字符串和创建 `JS` 对象到底谁快谁慢。我们只需要知道存在不同层面的计算：`JS` 层面的计算和 `DOM` 层面的计算，而两者的执行速度是完全不在一个数量级。
...
至此，我们对比了 `JS` 层面的计算和 `DOM` 层面的计算，速度完全不一样。
