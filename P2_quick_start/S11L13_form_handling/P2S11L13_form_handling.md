...
本例中，文本框和 `ref` 数据进行了绑定。当用户在输入内容时，`Vue` 会更新 `textContent` 这个 `ref` 数据，而 `ref` 数据的变化又会影响文本框本身的 `value` 值。这其实是一个 **双向绑定** 的示例。

虽然实现了双向绑定，但写起来比较麻烦，因此 `Vue` 提供了一个内置的指令 `v-model`：
...
1. 简化了书写；
2. 根据所使用的元素自动选择对应的属性以及事件组合——
   - `input` 或 `textarea` 元素：会绑定 `input` 事件，绑定的值是 `value`；
   - 单选框或复选框：绑定事件为 `change` 事件，绑定的值为 `checked`；
   - `select` 下拉列表：绑定 `change` 事件，绑定的值是 `value`。
...
单一的复选框，可以用 `v-model` 绑定一个 `ref` 类型的布尔值，布尔值为 `true` 表示选中，`false` 表示未选中：
...
在上面的例子中，布尔值 `true` 是选中，`false` 是未选中，但是选中与否所代表的值也是可以自定义的：
...
const hobby = ref([])
const arr = ref([
  { id: 'swim', label: '游泳', value: '游个泳' },
  { id: 'run', label: '跑步', value: '跑个步' },
  { id: 'game', label: '游戏', value: '玩个游戏' },
  { id: 'music', label: '音乐', value: '听个音乐' },
  { id: 'movie', label: '电影', value: '看个电影' }
])
...
label[for] {
  ...
  &:hover {
    background-color: rgba(238, 130, 238, 0.521);
  }
}
</style>
```

在上面的例子中，`checkbox` 所绑定的数据不再是一个布尔值，而是一个数组（集合），那么当该复选框被选中时，该复选框所对应的值就会被加入到数组里面（示例中的输出结果是区分勾选的先后顺序的）。



### 1.4 单选框

单选框和多选框都可以舍弃原生的 `name` 属性，利用响应式变量收集表单字段：
