...
1. `CLI`：`Command Line Interface`，翻译成中文叫做 **命令行接口**，主要通过输入一些文本命令来和计算机进行交互。
2. `GUI`：`Graphical User Interface`，即图形用户界面，用户使用一些图形元素（图表、按钮...）来和计算机进行交互。现在大多数时候都是采用 `GUI` 的方式和计算机进行交互，操作系统、办公软件、浏览器
...
- 语音交互
- 虚拟现实 `VR` 以及增强现实 `AR`
- 脑机接口（`BCI`，即 `Brain-Computer Interface`）
- 可穿戴设备
...
>
> 答：人机交互方式和执行效率是成反比。早期的 `CLI` 虽然用户体验最差，但是效率是最高。
...
2. 个人计算机时代
   ...
   - 1983年：微软推出 `MS-DOS`（微软磁盘操作系统），这是一个基于 `CLI` 的操作系统，成为个人电脑上的标准操作系统之一。
3. `GUI` 的出现与发展
...
>
> 答：`CLI` 配置的参数的优先级 **更高**，会覆盖配置文件里面的参数。
